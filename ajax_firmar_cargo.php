<?php
// Archivo: ajax_firmar_cargo.php (VALIDA TOKEN DE SESIÓN)
session_start();
...
try {
    ...
    $rol = $_POST['rol']; 
    $tipo_origen = $_POST['tipo_origen'] ?? 'canvas';
    $token_ingresado = $_POST['token'] ?? '';

    // --- VALIDACIÓN DE TOKEN ---
    if ($tipo_origen === 'registrada') {
        if (empty($token_ingresado)) {
            throw new Exception("Debe ingresar el Token de seguridad.");
        }
        if (!isset($_SESSION['token_firma']) || $_SESSION['token_firma'] != $token_ingresado) {
            throw new Exception("El Token ingresado es INCORRECTO o ha expirado.");
        }
        // Validar expiración (10 mins)
        if ((time() - $_SESSION['token_creado']) > 600) {
            unset($_SESSION['token_firma']);
            throw new Exception("El Token ha expirado. Solicite uno nuevo.");
        }
        unset($_SESSION['token_firma']); // Token consumido
    }
    ...
    if ($tipo_origen === 'registrada') {
        $stmtUser = $pdo->prepare("SELECT firma_imagen_path FROM usuarios WHERE id_usuario = ?");
        $stmtUser->execute([$id_usuario]);
        $firma_filename = $stmtUser->fetchColumn();
        $firma_origen = $ruta_firmas_destino . $firma_filename;

        if (empty($firma_filename) || !file_exists($firma_origen)) {
            throw new Exception("No tienes una firma registrada en tu perfil.");
        }
        if (!copy($firma_origen, $path_destino)) {
            throw new Exception("Error al procesar la firma del sistema.");
        }
    } else {
        if (empty($_POST['firma_base64'])) throw new Exception("No se recibió el trazo.");
        $img = $_POST['firma_base64'];
        $parts = explode(',', $img);
        $data = base64_decode(count($parts) > 1 ? $parts[1] : $parts[0]);
        if (!file_put_contents($path_destino, $data)) throw new Exception("Error al guardar imagen.");
    }
    ...
    if ($infoCargo['id_usuario_relevador']) {
        $pdo->prepare("INSERT INTO notificaciones (id_usuario_destino, tipo, mensaje, url, fecha_creacion) VALUES (?, 'aviso_firma', ?, ?, NOW())")->execute([$infoCargo['id_usuario_relevador'], $mensaje, $link]);
    }
    ...
    while ($admin = $stmtAdmins->fetch(PDO::FETCH_ASSOC)) {
        if ($admin['id_usuario'] != $infoCargo['id_usuario_relevador']) {
            $pdo->prepare("INSERT INTO notificaciones (id_usuario_destino, tipo, mensaje, url, fecha_creacion) VALUES (?, 'aviso_firma', ?, ?, NOW())")->execute([$admin['id_usuario'], $mensaje, $link]);
        }
    }
