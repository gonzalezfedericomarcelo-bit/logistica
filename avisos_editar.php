<?php
// Archivo: avisos_editar.php (CORREGIDO: CON FONT AWESOME)
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['imagen_destacada']) && $_FILES['imagen_destacada']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($ext, ['jpg','jpeg','png','webp'])) {
            ...
            if (move_uploaded_file($_FILES['imagen_destacada']['tmp_name'], 'uploads/avisos/' . $new_name)) {
                if ($imagen_actual && file_exists('uploads/avisos/' . $imagen_actual)) unlink('uploads/avisos/' . $imagen_actual);
    ...
    <div class="container mt-4 mb-5">
        <div class="card shadow-lg border-0">
            <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center py-3">
                <h4 class="mb-0"><i class="fas fa-edit me-2"></i>Editar Entrada</h4>
                <a href="avisos_lista.php" class="btn btn-sm btn-outline-light rounded-pill px-3">Volver a Gestión</a>
            </div>
            <div class="card-body p-4">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="text-end"><button type="submit" class="btn btn-primary btn-lg px-5"><i class="fas fa-save me-2"></i>Guardar Cambios</button></div>
                </form>
            </div>
        </div>
    </div>
